...
function loadData() {
    try {
        ...
        if(!appData.webdav) appData.webdav = { url: "", user: "", pass: "" };
...
function initSettings() {
    ...
    // 辅助：处理 WebDAV 请求的通用错误
    const handleDavError = (error) => {
        console.error("WebDAV Error:", error);
        if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
            return "浏览器安全策略(CORS)拦截了请求。\n请安装浏览器插件 'Allow CORS' 或使用自建 WebDAV 服务。";
        }
    ...
    document.getElementById('davUploadBtn').onclick = async () => {
        ...
        if(!url || !user || !pass) {
            davMsg.textContent = "请填写完整 WebDAV 信息";
            return;
        }
        ...
        try {
            davMsg.textContent = "正在连接...";
            davMsg.style.color = "#888";
            
            const targetUrl = safeUrl + 'mytab_backup.json';
            
            // 关键：使用 text/plain 避免部分服务器对 JSON 类型报错，
            // 且 utf8_to_b64 修复了中文密码问题
            ...
            if (response.ok || response.status === 201 || response.status === 204) {
                ...
                davMsg.style.color = "#4caf50";
            } else {
                throw new Error(`服务器拒绝: ${response.status} ${response.statusText}`);
            }
        } catch(e) { 
            davMsg.innerText = "失败: " + handleDavError(e);
            davMsg.style.color = "#ff6b6b";
        }
    };
    ...
    document.getElementById('davDownloadBtn').onclick = async () => {
        ...
        try {
            davMsg.textContent = "正在下载...";
            davMsg.style.color = "#888";

            const targetUrl = safeUrl + 'mytab_backup.json';

            // 关键：GET 请求不要带 Content-Type，部分服务器会因此拒绝
            ...
            if(res.ok) {
                ...
                if (res.status === 404) throw new Error("云端没有备份文件");
